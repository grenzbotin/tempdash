...
#define FIREBASE_PATH "/dht/room_name" // We use dht as prefix in frontend app, just change the room name
#define TIME_PERIOD_UPSTREAM 600000 // Defines in which time period we want to push to our Firebase
#define TIME_PERIOD_DISPLAY 5000 // Defines how often we want to print the sensor values on the screen
...
void loop() {
  ...
  // Printing humidity and temperature on OLED display
  OLED.clearDisplay();
  ...
  if (CONTROL >= TIME_PERIOD_UPSTREAM) {
    ...
    if (currentTime) {
      ...
      // We set our control variable to -TIME_PERIOD_DISPLAY after 
      // we pushed the value to Firebase
      CONTROL = -TIME_PERIOD_DISPLAY;
    }
  }

  delay(TIME_PERIOD_DISPLAY);
  CONTROL += TIME_PERIOD_DISPLAY;
}
